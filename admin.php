...
$formhash = substr(md5(substr(TIMESTAMP, 0, -7) . $username . $uid . ENCRYPT_KEY . ROOT . 'ADMINCP_ONLY'), 5, 14);
...
switch ($_GET['action']) {
    case 'load_userstat':
        ...
        $query = DB::query("SELECT `uid`, COUNT(*) AS `num` FROM `sign_log` WHERE `date` = {$date} AND `status` IN (1, 2, 3, 4) GROUP BY `uid`");
        while ($result = DB::fetch($query)) {
            $_uid = $result['uid'];
        ...
        $query = DB::query("SELECT `uid`, COUNT(*) AS `num` FROM `sign_log` WHERE `date` = {$date} AND `status` >= 10 GROUP BY `uid`");
        while ($result = DB::fetch($query)) {
            $_uid = $result['uid'];
            $data[$_uid]['unsupport'] = $result['num'];
        }
        $query = DB::query("SELECT `uid`, COUNT(*) AS `num` FROM `sign_log` WHERE `date` = {$date} AND `status` = 5 GROUP BY `uid`");
        while ($result = DB::fetch($query)) {
            $_uid = $result['uid'];
    ...
    case 'reset_failure':
        ...
        $date = date('Ymd');
        DB::query("UPDATE `sign_log` SET `status` = 0, `retry` = 0, `lastErr` = '' WHERE `uid` = {$_uid} AND `date` = {$date} AND `status` > 6");
