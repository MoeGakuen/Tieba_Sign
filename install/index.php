...
switch ($step) {
    default:
        ...
        $content .= '<p>本程序将会指引你在服务器上配置好“贴吧签到助手”</p>';
        $content .= '<p>点击右侧的“下一步”按钮开始</p><p class="btns"><button onclick="location.href=\'./?step=check\';">下一步 &raquo;</button>';
        show_install_page('Welcome', $content);
        break;

    case 'check':
        $content = '<p>安装前，程序需要检查当前的服务器环境是否允许运行“贴吧签到助手”</p>';
        $content .= '<p>请确保表格中每一行均为绿色，以避免可能带来的问题</p>';
        $content .= '<table><thead><tr><td>项目</td><td>要求</td><td>当前状态</td></tr></thead><tbody>';
    ...
    case 'install':
        ...
        if (empty($username) || empty($password) || empty($email)) show_back('注册账号', '您输入的信息不完整');
        ...
        $db_host = !isset($_POST['db_server']) ?: trim($_POST['db_server']);
        $db_port = !isset($_POST['db_port']) ?: intval($_POST['db_port']);
        ...
        $function = $db_pconnect ? 'mysql_pconnect' : 'mysql_connect';
        $link = $function("{$db_host}:{$db_port}", $db_username, $db_password);
        ...
        $version = preg_match('/version ([0-9a-z.]+)/i', $install_script, $match) ? trim($match[1]) : '';
        if (empty($version)) show_back('正在安装', '安装脚本有误，请重新上传');
        $err = runquery($install_script, $link);
        if (!empty($err)) show_back('正在安装', '安装过程出现错误:</p><p>' . $err);
        $syskey = random(32);
        $password = md5($syskey . md5($password) . $syskey);
