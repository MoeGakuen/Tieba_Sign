...
if (!$uid) {
    ...
    switch ($_GET['action']) {
        case 'skip_tieba':
            ...
            $tid = intval($_GET['tid']);
            $skiped = DB::result_first("SELECT `skiped` FROM `my_tieba` WHERE `tid` = {$tid}");
            $skiped = $skiped ? 0 : 1;
            $date = date('Ymd', TIMESTAMP + 900);
            if ($skiped == 1) {
                DB::query("UPDATE `sign_log` SET `status` = 5, `lastErr` = '用户手动忽略' WHERE `tid` = {$tid} AND `date` = {$date}");
            } else {
                DB::query("UPDATE `sign_log` SET `status` = 0, `lastErr` = '' WHERE `tid` = {$tid} AND `date` = {$date}");
            }
            DB::query("UPDATE `my_tieba` SET `skiped` = {$skiped} WHERE `tid` = {$tid}");
            if (!DB::affected_rows()) showmessage('发生未知错误: 无法修改贴吧设置');
            showmessage('修改签到设置成功！');
        case 'test_sign':
            if ($_GET['formhash'] != $formhash) break;
            $tieba = DB::fetch_first("SELECT * FROM `my_tieba` WHERE `uid` = {$uid} AND `skiped` = 0 ORDER BY RAND() LIMIT 0,1");
            if (!$tieba) showmessage('没有喜欢的贴吧，请先刷新喜欢的贴吧列表', './#liked_tieba');
            $setting = get_setting($uid);
            list($status, $result, $exp) = client_sign($uid, $tieba);
            $status = $status === 1 ? '签到成功' : '签到失败';
        ...
        case 'refresh_liked_tieba':
            ...
            showmessage("喜欢的贴吧列表已经更新,<br>新增<b>{$insert}</b>个贴吧, 删除<b>{$deleted}</b>个贴吧", './#liked_tieba', 1);
            break;
    }
    header('Location: ./');
    exit();
