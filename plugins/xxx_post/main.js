...
$("#xxx_post_add_tid").click(function() {
    createWindow().setTitle("添加帖子").setContent('<p>你可以指定帖子进行回复</p><p>请输入帖子的地址:</p><p>例如:http://tieba.baidu.com/p/2692275116</p><form method="get" action="plugin.php?id=xxx_post&action=get-tid" id="xxx_post_tid_form" onsubmit="return post_win(this.action, this.id,x_reload)"><input type="text" id="xxx_post_tid" name="xxx_post_tid" style="width:90%"/></form>').addButton("确定",
    function() {
        $('#xxx_post_tid_form').submit();
    }).addCloseButton("取消").append();
});
$("#x_p_add_tb").click(function() {
    createWindow().setTitle("添加帖吧").setContent('<p>你可以只指定贴吧，并从该贴吧首页随机选择帖子进行回复</p><p>请输入帖吧的名字（不要带“吧”字）:</p><p>例如:要添加chrome吧，请输入chrome</p><form method="get" action="plugin.php?id=xxx_post&action=add-tieba" id="xxx_post_add_tb_form" onsubmit="return post_win(this.action, this.id,x_reload)"><input type="text" id="xxx_post_add_tieba" name="xxx_post_add_tieba" list="autocomplete-tieba" style="width:90%"/></form>').addButton("确定",
    function() {
        $('#xxx_post_add_tb_form').submit();
    }).addCloseButton("取消").append();
});
$("#xxx_post_add_content").click(function() {
    createWindow().setTitle("添加回帖内容").setContent('<p>请输入要回复的内容（最多1000字符）:</p><form method="get" action="plugin.php?id=xxx_post&action=set-content" id="xxx_post_content_form" onsubmit="return post_win(this.action, this.id,x_reload)"><textarea name="post_content" id="post_content" rows="5" style="width: 95%"></textarea></form>').addButton("确定",
    function() {
        $('#xxx_post_content_form').submit();
    }).addCloseButton("取消").append();
});
$("#x_p_add_con").click(function() {
    createWindow().setTitle("批量添加内容").setContent('<p>请输入要回复的内容（每行算一条）:</p><form method="get" action="plugin.php?id=xxx_post&action=set-cont-plus" id="x_p_cont_form" onsubmit="return post_win(this.action, this.id,x_reload)"><textarea name="x_p_contant" id="x_p_contant" rows="8" style="width: 95%"></textarea></form>').addButton("确定",
    function() {
        $('#x_p_cont_form').submit();
    }).addCloseButton("取消").append();
});
$("#x_p_del_con").click(function() {
    createWindow().setTitle("批量删除").setContent('你确定要删除全部回复内容吗？').addButton("确定",
    function() {
        msg_callback_action('plugin.php?id=xxx_post&action=del-all-cont', x_reload);
    }).addCloseButton("取消").append();
});
$("#x_p_del_tid").click(function() {
    createWindow().setTitle("批量删除").setContent('你确定要删除全部贴子吗？').addButton("确定",
    function() {
        msg_callback_action('plugin.php?id=xxx_post&action=del-all-tid', x_reload);
    }).addCloseButton("取消").append();
});
...
function load_post_set() {
    showloading();
    $.getJSON("plugin.php?id=xxx_post&action=post-settings",
    function(result) {
        show_post_set(result);
    }).fail(function() {
        createWindow().setTitle('系统错误').setContent('发生未知错误: 无法获取设置').addButton('确定',
        function() {
            location.reload();
        }).append();
    }).always(function() {
...
function load_post_adv_set() {
    showloading();
    $.getJSON("plugin.php?id=xxx_post&action=post-adv-settings",
    function(result) {
...
function load_post_log() {
    showloading();
    $.getJSON("plugin.php?id=xxx_post&action=post-log",
    function(result) {
        show_post_log(result);
    }).fail(function() {
        createWindow().setTitle('系统错误').setContent('发生未知错误: 无法获取回帖报告').addButton('确定',
        function() {
            location.reload();
        }).append();
    }).always(function() {
...
function load_post_history(date) {
    showloading();
    $.getJSON("plugin.php?id=xxx_post&action=post-history&date=" + date,
    function(result) {
        show_post_log(result);
    }).fail(function() {
        createWindow().setTitle('系统错误').setContent('发生未知错误: 无法获取签到报告').addButton('确定',
        function() {
            location.reload();
        }).append();
    }).always(function() {
...
function delsid(sid) {
    createWindow().setTitle('删除帖子').setContent('确认要删除这个帖子的自动回复吗？').addButton('确定',
    function() {
        msg_callback_action("plugin.php?id=xxx_post&action=delsid&sid=" + sid, x_reload);
    }).addCloseButton('取消').append();
    return false;
}
function delcont(cid) {
    createWindow().setTitle('删除帖子').setContent('确认要删除这个回复内容吗？').addButton('确定',
    function() {
        msg_callback_action("plugin.php?id=xxx_post&action=delcont&cid=" + cid, x_reload);
    }).addCloseButton('取消').append();
    return false;
}